          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Institutional authority mapping, verified direct communication channels, and outreach cadence.
              </p>
            </div>
          </div>
        </div>
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-border">
              {contacts.map((contact) => (
                <div key={`mobile-${contact.id}`} className="p-4 space-y-3 hover:bg-surface-hover transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      <div className="flex size-8 shrink-0 items-center justify-center rounded-md bg-surface-muted border border-border text-foreground font-semibold text-xs font-mono">
                    ...
                    {contact.isDecisionMaker && (
                      <span className="shrink-0 inline-flex items-center gap-1 font-mono text-[9px] font-semibold px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                        <Star className="size-2.5 text-amber-500" /> DM
                      </span>
                    )}
                  </div>
                  ...
                  <div className="text-xs">
                    <Link
                      ...
                      className="font-medium text-xs text-primary hover:underline inline-flex items-center gap-1.5"
                    >
                  ...
                  {(contact.phone || contact.email) && (
                    <div className="flex flex-wrap items-center gap-3 text-xs font-mono text-[11px] pt-1 border-t border-border text-muted-foreground">
                      {contact.phone && (
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-border text-[11px] font-semibold text-muted-foreground bg-surface-muted/50">
                    ...
                    <th className="py-3 px-3.5 font-semibold">Role & Department</th>
                    <th className="py-3 px-3.5 font-semibold">Organization</th>
                    <th className="py-3 px-3.5 font-semibold">Authority Level</th>
                    <th className="py-3 px-3.5 font-semibold">Direct Phone</th>
                    <th className="py-3 px-3.5 font-semibold">Direct Email</th>
                ...
                <tbody className="divide-y divide-border/60">
                  {contacts.map((contact) => (
                    <tr
                      key={contact.id}
                      className="group hover:bg-surface-hover transition-[background-color] duration-100"
                    >
                      <td className="py-3 px-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="flex size-7.5 shrink-0 items-center justify-center rounded-md bg-surface-muted border border-border text-foreground font-semibold text-xs font-mono">
                            {contact.name.slice(0, 2).toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <span className="font-semibold text-xs text-foreground block truncate">
                      ...
                      <td className="py-3 px-3.5 text-xs text-muted-foreground">
                      ...
                      <td className="py-3 px-3.5">
                        {contact.isDecisionMaker ? (
                          <span className="inline-flex items-center gap-1 font-mono text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                      ...
                      <td className="py-3 px-3.5 text-xs font-mono text-[11px]">
                        {contact.phone ? (
                      ...
                      <td className="py-3 px-3.5 text-xs font-mono text-[11px] truncate max-w-[190px]">
                        {contact.email ? (
        ...
        {pageCount > 1 && (
          <div className="flex items-center justify-between border-t border-border px-4 py-2.5 text-xs text-muted-foreground bg-surface-muted/30">
            <div className="font-mono text-[11px]">
              Showing page <span className="text-foreground font-semibold">{page}</span> of {pageCount} ({total} contacts)
            </div>
